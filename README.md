...
1.  **Source-Only (Baseline):** Trained on PDB data, tested on AlphaFold data (Lower bound).
2.  **Target-Only (Oracle):** Trained and tested on AlphaFold data (Upper bound).
3.  **Domain Adaptation Model (ADDA/DANN):** Feature extractor alignment between domains.
4.  **Ensemble / Variant:** (e.g., Bagging Classifier or Augmented MLP).
